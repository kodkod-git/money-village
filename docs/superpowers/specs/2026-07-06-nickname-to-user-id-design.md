...
- 기존 테스트(`tests/player-edit-nickname-rename.test.js` 등)를 `user_id` 기준 로직에 맞게 갱신
- 닉네임 변경 후 자산/특성/은행/퀴즈 기록이 유지되는지 확인하는 테스트 추가
- 명예의 전당 팀 순위(`team_total_asset`, `members`)가 온더플라이 계산 후에도 기존과 동일한 값을 내는지 확인하는 테스트 추가
