...
assert(
  /#bankScreen \.bank-screen-wrap,\s*#luckScreen \.bank-screen-wrap,\s*#quizView2 > \.bank-screen-wrap\s*\{[\s\S]*height:\s*calc\(100vh - 48px\);[\s\S]*height:\s*calc\(100dvh - 48px\);/.test(css),
...
assert(
  /#bankPlayerGrid,\s*#quizPlayerGrid,\s*#luckPlayerGrid\s*\{[\s\S]*flex:\s*1 1 auto;[\s\S]*overflow-y:\s*auto;[\s\S]*-webkit-overflow-scrolling:\s*touch;/.test(css),
  'bank, quiz, and luck player grids should share touch-friendly internal scrolling'
);

assert(
  /#bankView2,\s*#quizView2,\s*#luckView2\s*\{[^}]*display:\s*flex;[^}]*flex-direction:\s*column;[^}]*overflow:\s*hidden;[^}]*\}/.test(css),
...
assert(
  /#bankPlayerGrid,\s*#quizPlayerGrid,\s*#luckPlayerGrid\s*\{[^}]*padding:\s*12px 0 10px;/.test(css),
